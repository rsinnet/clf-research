...
% Added in change for compatibility with old code (switch for 'R[axis]M' -
...
switch code
  ...
  case {'R','Rz'}			% revolute Z axis
    Xj = rotz(q);
    S = [0;0;1;0;0;0];
  case {'-Rx', 'RxM'}				% revolute X axis (minus)
    Xj = rotx(-q);
    S = [-1;0;0;0;0;0];
  case {'-Ry', 'RyM'}				% revolute Y axis (minus)
    Xj = roty(-q);
    S = [0;-1;0;0;0;0];
  case {'-Rz', 'RzM'}			% revolute Z axis (minus)
    Xj = rotz(-q);
    S = [0;0;-1;0;0;0];
