classdef hmodel3d < hsys.hmodelsva
%%%HMODEL3D
...
%
    ...
    methods        
        function this = hmodel3d()
        % Default constructor
        %xdir = fileparts(mfilename('fullpath'));
        %xdir = pwd;
        %rmpath(fileparts(which('pnst')));
        %addpath([xdir filesep 'build_old-3d-feet-amber-suite']);
        %setup_toolbox_path(xdir);
            ...
            P1 = plotposz(zeros(this.nExtDof, 1), [], -1);
        ...
        % Embed from restricted guard space to full coordinates.
            ...
            qf  = [zeros(nb-1,1); x(1); x(2); qi(1); 0; x(3);
                   x(4);  0;  x(5); qi(2)];
            dqf = [zeros(nb-1,1); x(6); x(7);  x(8); 0; x(9);
                   x(10); 0; x(11); x(12)];
